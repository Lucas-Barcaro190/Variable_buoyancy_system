...
class VBSSensorCharacterization:
    ...
    def create_read_depth_sensor_commands(self, repeat: int) -> List[Tuple[str, int]]:
        """Create commands to read depth sensor repeatedly"""
        commands = []
        for _ in range(repeat):
            commands.extend(self.vbs_command(1, "depth_sensor", wait_time=100))
        commands.extend(self.vbs_command(1, " ", wait_time=60000))  # Add a pause after readings
        return commands
    ...
    def run_sequence(self):
        ...
        sequence_phases = [
            ...
            {
                "message": "!!!!"    ,
                "commands": sum([self.create_step_commands_minimal(2 * 5, value=-28) for _ in range(30)], []),  
                "user_input": None
            },
            {
                "message": "!!!!"    ,
            ...
            {
                "message": "!!!!"    ,
                "commands": sum([self.create_step_commands_minimal(2 * 5, value=-56) for _ in range(30)], []),  
                "user_input": None
            },
            {
                "message": "!!!!"    ,
                ...
                "message": "!!!!"    ,
                "commands": sum([self.create_step_commands_minimal(2 * 5, value=-86) for _ in range(30)], []),  
                "user_input": None
            },
            {
                "message": "!!!!"    ,
