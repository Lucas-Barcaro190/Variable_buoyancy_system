...
//    Essa função pode ser chamada checkAndHandleFaults() e deve verificar os flags de sysFault e sysError
...
//  
// 3) Implement the fibonacci indentation for better understanding (makes sure that nests don't occur)
...
volatile uint8_t driverResponse[64];
volatile int driverResponseLen = 0;
...
volatile bool potentiometerSemaphore = false;
...
void handleMoveCommand(char* args) {
    ...
    */
    ...
    if (speedVal > MAX_SPEED_VAL) speedVal = MAX_SPEED_VAL; // caps speed at max value
    ...
    if (!direction && faultResult != SW_MAX_LIMIT_ERROR) {
        sysError = 0;
        sysFault = false;
    }
...
void handleMovePotCommand(char* args) {
    ...
    behavior:
     ...
     - It checks the sysFault and sysError flags to prevent movement in the direction of a triggered limit switch,
    ...
    */
    ...
    char* nextPtr;
    long potValue = strtol(args, &nextPtr, 10);
    int speedRaw = (int)strtol(nextPtr, NULL, 10);
    ...
    uint8_t direction = (potentiometerValue > potValue) ? 1 : 0;
    uint8_t speedVal = (uint8_t)abs(speedRaw);

    if (speedVal > MAX_SPEED_VAL) speedVal = MAX_SPEED_VAL;
    
    uint8_t val = (direction << 7) | speedVal;
    ...
    while(potentiometerValue < potValue - 3 || potentiometerValue > potValue + 3){ // pot value +- 3 of error (around 0.13mm)
        vTaskDelay(pdMS_TO_TICKS(50)); // Delay to allow movement and potentiometer update
    }

    // Send stop
    uint8_t stopPacket[2];
    stopPacket[0] = DRIVER_ADDR;
    stopPacket[1] = 0xF7;
    
    sendPacketWithChecksum(stopPacket, 2);
    
    printf("\n[Pico -> Driver]: Stop\n");
    printf("\nTarget potentiometer value %ld reached. Current value: %d\n", potValue, potentiometerValue);
}
...
int8_t treatCustomCommand(const char* cmdName, char* args = NULL) {
    ...
    behavior:
     - Checks if the cmdName matches any of the custom commands (e.g. "move").
    ...
    */
    ...
    if (strcmp(cmdName, "move") == 0) {
        ...
        if (strcmp(cmdName, "move_until_pot") == 0) {
        handleMovePotCommand(args); // handles move_until_pot command separately
        return 0;
    }
    
        if (strcmp(cmdName, "full_contract") == 0) {
        handleMovePotCommand("44 2\0"); // contracts until potentiometer is around +-2 of 44
        return 0;
    }
    
    if (strcmp(cmdName, "full_expand") == 0) {
        handleMovePotCommand("434 2\0"); // expands until potentiometer is around +-2 of 434
        return 0;
    }
    
    if (strcmp(cmdName, "send_status") == 0 && !potentiometerSemaphore) {
        potentiometerSemaphore = true;
        if (potentiometerValue < MINIMAL_THRESHOLD + 5) {
            printf("\n[VBS]: Fully contracted.\n");
        }
        else{
            if( potentiometerValue > MAXIMUM_THRESHOLD - 5) {
                printf("\n[VBS]: Fully expanded.\n");
            }
            else{
                printf("\n[VBS]: In movement.\n");
        ...
        potentiometerSemaphore = false;
        return 0;
    }
    return NOT_CUSTOM_COMMAND;
}
...
void treatSimpleCommand(const char* cmdName) {
    /*
    params:
     - cmdName: The namoe of the command to check and execute if it matches a simple command.
    behavior:
     - Checks if the cmdName matches any of the predefined simple commands in the simple Commands array.
     - If it matches, it sends the corresponding byte sequence to the driver using sendPacketWithChecksum.
     - If it does not match any simple command, it returns without doind anything, allowing the caller to print an error message.
    */
    for (int i = 0; i < numSimpleCommands; i++) {
...
static absolute_time_t lastInterruptTime;
...
void vPotentiometerTask(void *pvParameters) {
    ...
    */
    ...
    while (true) {
        ...
        if (!PotentiometerSemaphore) {
            potentiometerSemaphore = true;
            potentiometerValue = mean;
            potentiometerSemaphore = false;
        }
    ...
    */
    ...
    while (true) {
        if (xQueueReceive(xUartQueue, &byte, pdMS_TO_TICKS(20))) { // Waits for a byte with a timeout of 20ms
            ...
            driverResponseLen = rxIndex;
            memcpy((void*)driverResponse, rxBuffer, driverResponseLen); // Copy received message
